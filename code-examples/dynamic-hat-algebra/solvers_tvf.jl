...
function solve_temp_equilibrium(
    ...
    while (itw <= ap.maxit) & (ωmax > ap.tol_in)
        ...
        phat, xhat = solve_prices(phat, ω, ω0, κ_hat, Z_hat, θinv, θinv_stack, G, G_vec, γ, λ, λ_0, λ_1, mp, ap)

        # calculate bilateral trade shares [eq (13) on page 754]
        λ_prime = solve_λprime(λ, λ_0, λ_1, κ_hat, Z_hat, xhat, phat, θinv, θinv_stack, γ, mp)
    ...
    # recovering wages and rental rates frω matrix of factor prices ω
    wages = similar(ω)
...
function solve_λprime(λ, λ_0, λ_1, κ_hat, Z_hat, xhat, phat, θinv, θinv_stack, γ, mp)

    # Calculating bilateral trade shares using eqn (13)
    xhat_prime = xhat.^(-1 ./ θinv)
    phat_prime = phat.^(-1 ./ θinv)

    λ_k = ( λ_1 ./ λ_0 ) .* λ .* (κ_hat.^(-1 ./ θinv_stack))
    λ_k[isnan.(λ_k)] .= 0
...
function solve_prices(phat_guess, ω, ω0, κ_hat, Z_hat, θinv, θinv_stack, G, G_vec, γ, λ, λ_0, λ_1, mp, ap)
