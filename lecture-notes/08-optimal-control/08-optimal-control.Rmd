---
title: "Lecture 8"
subtitle: "Continuous time dynamic models"
author: Ivan Rudik
date: AEM 7130
output:
...
```{julia, echo = FALSE, fig.align = 'center'}
using Plots
plot(t1, y1,
    linestyle = [:solid],
    linewidth = [3],
...
plot([t1 t2 t1], [y1 y2 y_real], 
  ...
  xlabel = "t",
  ylabel = "f(t)",
  label = ["Euler" "Implicit Euler" "exp(t)"])
```
...
```{julia, echo = FALSE, fig.align = 'center'}
plot([t1 t2 t1 t2], [y1 y2 y3 y_real], 
  linestyle = [:dot :dash :dashdot :solid],
  linecolor = [:red :blue :orange :purple],
  tickfontsize = 12,
  grid = :yes,
  legend = :topleft,
  xlabel = "t",
  ylabel = "f(t)",
  label = ["Euler" "Implicit Euler" "RK" "exp(t)"])
```
...
```{julia, echo = FALSE, fig.align = 'center'}
plot([t1 t2 t1], [log10.(abs.(y1.-y_real)./y_real) log10.(abs.(y2.-y_real)./y_real) log10.(abs.(y3.-y_real)./y_real)], 
  linestyle = [:dot :dash :dashdot],
  linecolor = [:red :blue :orange],
  tickfontsize = 12,
  grid = :yes,
  legend = :topleft,
  xlabel = "t",
  ylabel = "log10 Relative Error",
  label = ["Euler" "Implicit Euler" "RK"])
```
...
\begin{align}
  \dot{A}(t) = f(A(t)) + w(t) - c(t) \qquad \dot{c}(t) = -\frac{u'(c(t))}{u''(c(t))}\left[f'(A(t)) - r\right]
\end{align}
...
We will guess the terminal condition and integrate .hi-blue[backward]
...
```
```{julia, echo = FALSE}
plot([t t t t], [k aend*ones(size(k)) c cend*ones(size(c))], 
  linestyle = [:solid :dot :solid :dot],
  linecolor = [:red :purple :green :blue],
  tickfontsize = 12,
  grid = :yes,
  legend = :right,
  xlabel = "t",
  ylabel = "Assets / consumption",
  label = ["Assets" "Steady State Assets" "Consumption" "Steady State Consumption"])
```
