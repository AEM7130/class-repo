...
pacman::p_load(
  xaringanExtra, JuliaCall, tweetrmd
)
...
You .hi-blue[must] use Julia and write .jl scripts, no Jupyter notebooks
...
.pull-right[
```{r aea-tweet, echo = FALSE, fig.pos="c"}
tweet_screenshot(
  "https://twitter.com/seema_econ/status/1345858063622836224?s=20"
  )
```
]
...
The final project will be the beginning of a .hi-blue[computationally-driven] research project or an extension of an existing paper with new methods
...
.hi-blue[bubble sort], step through a list, compare adjacent elements, swap if in the wrong order
