...
1. Learning how to compute dynamic and spatial models
2. Other useful computational techniques and details
...
1. Everything we use in the course will be .hi[freely available] and posted to the course GitHub (details next class on how to use Git)
...
- Spend some time reading up on Julia if you don't use it already:
...
1. Basic computing, arithmetic, calculus, and linear algebra on a computer
2. Shell and Julia coding, version control, reproducibility, workflow
3. Optimization
...
- Final project proposal: XYZ
- Final project paper: XYZ
...
You .hi[must] use Julia
  - Code must be written in `.jl` scripts
  - You must use Julia project management tools, e.g. `Pkg.generate(), Pkg.activate(), Pkg.instantiate()`
  - Everything must be nested in a wrapper function
  - It must work just by running the wrapper function

You can work in groups of 2

Problem sets will be where you .hi[implement] the techniques we learn in class on your own,
...
Everyone will present a paper or package starting in a few weeks

The paper can apply methods we've learned about (or will learn about),
or can be a new method that we have not covered

The package must be something related to the methods we are learning

You must consult with me at least 1 week prior to your scheduled presentation
date to ensure the paper/package is appropriate for a presentation

The syllabus has some pre-approved papers you can choose from under the .hi[Applications] header

---

# Final project (25% paper, 15% proposal)

The final project will be the beginning of a .hi[computationally-driven] research project or an extension of an existing paper with new methods
...
The only requirement is that the project .hi[cannot be computationally trivial]  
(i.e. no reduced form applied micro papers)
...
All slides will be available on the [course GitHub page](https://github.com/AEM7130/class-repo)
...
Not all economic models have closed-form solutions,
and others can't have closed-form solutions with losing important economic content

This is generally true for dynamic and spatial models

---

# What can we compute?

We can use computation + theory to answer .hi[quantitative] questions
...
- strictly linear transitions (natural phenomena don't follow this)
...
.hi[What price clears the market in equilibrium?]
...
.hi[How much acreage does the farmer plant?]
...
We can solve this using another technique called .hi[function iteration]
...
i.e. what is the .hi[computational complexity] of the problem?

--

.hi[General mathematical definition:] Big O notation describes the limiting behavior of a function when the argument tends towards a particular value or infinity

--

.hi[Programming context:] Describes the limiting behavior of algorithms in terms of run time/memory/accuracy as input size grows
...
**O(x):** linear
- Time to solve increases linearly in size of input x
- Accuracy changes linearly in size of input x
...
**O(1):** algorithm executes in .hi[constant time]
...
**O(x):** algorithm executes in .hi[linear time]
...
$\mathbf{O(x^2):}$ algorithm executes in .hi[quadratic time]
...
.hi[bubble sort], step through a list, compare adjacent elements, swap if in the wrong order
...
.hi[Question:] which numbers can be represented by a computer?
...
.hi[Binary:] a base 2 number system

Each digit can only take on 0 or 1

--

.hi[Base 10:] each digit can take on 0-9
...
.hi[Answer:] a subset of the rational numbers

--

Computers have .hi[finite] memory and hard disk space, there are infinite rational numbers
...
`Int64` means it is a .hi[integer] with 64 bits of storage

`Float64` means it is a .hi[floating point number] with 64 bits of storage
...
.hi[1.] There exists a .hi[machine epsilon] which denotes the smallest relative quantity representible by a computer
...
Theres a tradeoff between precision and storage requirements,  
...
.hi[2.] There is a .hi[smallest representable number]
...
.hi[3.] There is a .hi[largest representable number]
...
Error comes in two major and related forms:
...
Adding numbers of greatly different magnitudesdoes not always work like you would want
...
Derivatives are important in economics for finding optimal allocations, etc
...
```{julia}
println("
        The deriviative with h=1e-8 is: $(deriv_x_squared(1e-8,2.))
        The deriviative with h=1e-12 is: $(deriv_x_squared(1e-12,2.))
        The deriviative with h=1e-30 is: $(deriv_x_squared(1e-30,2.))
        The deriviative with h=1e-1 is: $(deriv_x_squared(1e-1,2.))")
```
...
We want $h$ to be as small as possible so thatwe can approximate the limit as well as we possibly can, BUT
...
There are proofs for why this is the case but generally testing out different $h$'s works fine
...
.hi[Forward differences have linear errors]
...
We want the derivative at $x$, which is on the edge of $[x,x+h]$, how about we .hi[center] $x$?
...
.hi[Is this an improvement on forward differences?]
...
Coding up analytic derivatives by hand forcomplex problems is not always great because
...
Think about this: your code is .hi[always] made up of simple arithmetic operations
...
Exact solutions without handcoding
...
$\int_D f(x) dx$, $f:\mathcal{R}^n \rightarrow \mathcal{R}$, $D\subset\mathcal{R}^n$
...
Suppose we want to integrate $\xi = \int_0^1 f(x) dx$

How do we do it?

--

We can do so by drawing $N$ uniformly distributed samples, $x_1,...,x_N$ over interval $[0,1]$
...
The variance of $\hat{\xi}$ is $$\sigma^2_{\hat{\xi}} = \mathrm{var}\left(\frac{V}{N}\sum_{i=1}^N f(x_i)\right) = \frac{V^2}{N^2} \sum_{i=1}^N \mathrm{var}(f(X)) = \frac{V^2}{N}\sigma^2_{f(X)}$$
...
We can also approximate integrals using a technique called .hi[quadrature]
...
One answer is to replace the function with something easy to integrate: .hi[a piecewise polynomial]
...
$x_i$s are the .hi[quadrature nodes] of the approximation scheme and divide the interval into $n-1$ equally spaced subintervals of length $h$

---

# Midpoint rule

The most basic Newton-Cotes method:
...
Trapezoid rule is $O(h^2)$ aka first-order exact: it can integrate any linear function exactly

--

Seems sensible, a piecewise linear function can approximate any linear function exactly since it has more flexibility
...
Let $n$ be odd, then approximate the function across a .hi[pair] of subintervals
...
How accurate do you think Simpson's rule is?

--

Simpson's rule is $O(h^4)$ aka third-order exact: it can integrate any cubic function exactly

--

Why do we gain 2 orders of accuracy when increasing one order of approximation complexity?
...
3. This cubic error is .hi[odd] with respect to the midpoint: integrating over the first subinterval cancels integrating over the second subinterval, the integration error is zero
...
Gaussian quadrature selects these nodes more efficiently and relies on .hi[weight functions] $w(x)$
...
For a given order of approximation $n$, the weights $w_1,...,w_n$ and nodes $x_1,...,x_n$ are chosen to satisfy $2n$ .hi[moment-matching conditions]:
...
Given an approximation with $n$ mass points, $X$ and $\bar{X}$ have identical moments up to order $2n-1$,
...
We can start out with a simple $w(x) = 1$, this gives us .hi[Gauss-Legendre] quadrature
...
How do we .hi[actually] solve these systems inside the machine?
...
When we then perform the second step of backwards substitution, we solve for $x_1=-M(1-x_2)=0$, this is .hi[very] wrong
...
Approximate the elasticity by computing the .hi[condition number]
\begin{gather*}
	\kappa = ||A|| \cdot ||A^{-1}||
\end{gather*}
...
$\kappa$ gives the least upper bound of the elasticity: it is always larger than one and a rule of thumb is that for every order of magnitude, a significant digit is lost in the computation of $x$
