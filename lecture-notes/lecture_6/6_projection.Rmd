...
The partial derivative weight function solves the least squares problem!
...
We can solve this problem by *iterating* on the problem, continually setting the residuals equal to zero, recovering new $\theta$s, and repeating
...
Ideally we want an .hi-blue[orthogonal basis:] when we add another element of the basis,
...
that return the values of the approximant at `x`
...
We can obtain $3(N-1)$ equations by ensuring that the approximant is continuous at all interior knots,
...
where `evaluation_points` are the points we want to evaluate the spline approximant at
