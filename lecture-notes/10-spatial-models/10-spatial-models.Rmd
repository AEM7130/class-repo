...
The gravity model is widely used and empirically matches the data

.hi[But:] no real theoretical foundation, can't do counterfactuals
...
function solve_armington_eq(σ, τ, A, L, a, tol = 1e-5, damp = .1)
    w, λ, N  = ones(size(A)), zeros(size(a)), length(A)
    wage_error = 1e5
...
function solve_armington_exact_hat(X, λ, w, L, Ahat, σ, tol = 1e-5, damp = .1)
    what, N = ones(size(Ahat)), length(Ahat)
    wage_error = 1e5
    while wage_error > tol
